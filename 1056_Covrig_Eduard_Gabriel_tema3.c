...
void dezalocareHash(HashTable* tabela)
{
    for (int i = 0; i < (*tabela).dimensiune; i++)
    {
        ...
        while (curent != NULL)
        {
            ...
            free(curent->info.strada);
            free(curent->info.propietar);
            if (curent->info.lista_locuitori != NULL)
            {
                for (int j = 0; j < curent->info.locuitori; j++)
                {
                    free(curent->info.lista_locuitori[j]);
                }
                free(curent->info.lista_locuitori);
            }
            Nod* copie = curent;
            curent = curent->next;
            free(copie);
